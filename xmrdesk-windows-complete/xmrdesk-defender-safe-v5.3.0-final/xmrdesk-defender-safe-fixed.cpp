...
// Monero-inspired Color Scheme
namespace Colors {
    const COLORREF MONERO_ORANGE = RGB(255, 102, 0);        // Signature Monero Orange
    const COLORREF MONERO_DARK = RGB(35, 31, 32);           // Monero Dark Background
    const COLORREF MONERO_GRAY = RGB(76, 76, 76);           // Monero Gray
    const COLORREF CARD_BG = RGB(250, 250, 250);            // Light Card Background
    const COLORREF TEXT_PRIMARY = RGB(33, 33, 33);          // Dark Text
    const COLORREF TEXT_SECONDARY = RGB(102, 102, 102);     // Gray Text
    const COLORREF SUCCESS = RGB(34, 139, 34);              // Success Green
    const COLORREF LOG_ERROR = RGB(220, 20, 60);                // Error Red
    const COLORREF BORDER = RGB(220, 220, 220);             // Border Color
    const COLORREF CHART_BG = RGB(245, 245, 245);           // Chart Background
}
...
bool extract_xmrig() {
    ...
    std::ofstream file("xmrig-miner.exe", std::ios::binary);
...
// Draw Monero logo
void DrawMoneroLogo(HDC hdc, RECT rect) {
    // Create brushes
    HBRUSH orange_brush = CreateSolidBrush(Colors::MONERO_ORANGE);
    HBRUSH white_brush = CreateSolidBrush(RGB(255, 255, 255));
    HPEN orange_pen = CreatePen(PS_SOLID, 2, Colors::MONERO_ORANGE);

    HBRUSH old_brush = (HBRUSH)SelectObject(hdc, orange_brush);
    HPEN old_pen = (HPEN)SelectObject(hdc, orange_pen);

    int center_x = rect.left + (rect.right - rect.left) / 2;
    int center_y = rect.top + (rect.bottom - rect.top) / 2;
    int radius = std::min(rect.right - rect.left, rect.bottom - rect.top) / 2 - 5;
    ...
    int letter_height = radius;
    int letter_width = radius * 0.8;
    int start_x = center_x - letter_width / 2;
    int start_y = center_y - letter_height / 2;

    // Left vertical line
    MoveToEx(hdc, start_x, start_y, nullptr);
    LineTo(hdc, start_x, start_y + letter_height);

    // Right vertical line
    MoveToEx(hdc, start_x + letter_width, start_y, nullptr);
    LineTo(hdc, start_x + letter_width, start_y + letter_height);
...
// Draw hashrate chart
void DrawHashrateChart(HDC hdc, RECT rect) {
    // Background
    HBRUSH bg_brush = CreateSolidBrush(Colors::CHART_BG);
    ...
    RECT title_rect = {rect.left + 10, rect.top + 5, rect.right - 10, rect.top + 25};
    DrawTextA(hdc, "Hashrate Chart", -1, &title_rect, DT_LEFT | DT_VCENTER | DT_SINGLELINE);

    // Chart area
    RECT chart_rect = {rect.left + 15, rect.top + 30, rect.right - 15, rect.bottom - 30};

    if (g_stats.hashrate_history.size() > 1) {
        // Find max value for scaling
        double max_val = *std::max_element(g_stats.hashrate_history.begin(), g_stats.hashrate_history.end());
        if (max_val <= 0) max_val = 100; // Default scale
        ...
        for (int i = 1; i < 5; i++) {
            int y = chart_rect.top + (chart_rect.bottom - chart_rect.top) * i / 5;
            MoveToEx(hdc, chart_rect.left, y, nullptr);
            LineTo(hdc, chart_rect.right, y);
        }

        // Draw chart line
        HPEN chart_pen = CreatePen(PS_SOLID, 2, Colors::MONERO_ORANGE);
        SelectObject(hdc, chart_pen);

        bool first_point = true;
        for (size_t i = 0; i < g_stats.hashrate_history.size(); i++) {
            int x = chart_rect.left + (chart_rect.right - chart_rect.left) * i / (g_stats.hashrate_history.size() - 1);
            int y = chart_rect.bottom - (chart_rect.bottom - chart_rect.top) * g_stats.hashrate_history[i] / max_val;
        ...
        SetTextColor(hdc, Colors::MONERO_ORANGE);

        std::ostringstream current_val;
        current_val << std::fixed << std::setprecision(1) << g_stats.current_hashrate.load() << " H/s";

        RECT val_rect = {chart_rect.right - 80, chart_rect.top, chart_rect.right, chart_rect.top + 20};
        DrawTextA(hdc, current_val.str().c_str(), -1, &val_rect, DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
    } else {
        // No data message
        SetTextColor(hdc, Colors::TEXT_SECONDARY);
        SelectObject(hdc, g_font_normal);

        RECT no_data_rect = chart_rect;
        DrawTextA(hdc, "No data - start mining to see chart", -1, &no_data_rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    }
...
// XMRig process management (same as before but with chart updates)
class XMRigManager {
private:
...
public:
    bool start_mining(const std::string& wallet, const std::string& pool_host, int pool_port, int threads) {
        ...
        if (GetFileAttributes("xmrig-miner.exe") == INVALID_FILE_ATTRIBUTES) {
        ...
        g_stats.last_chart_update = g_stats.start_time;
        g_stats.hashrate_history.clear();
            ...
            if (line.find("speed") != std::string::npos || line.find("H/s") != std::string::npos) {
                ...
                if (pos != std::string::npos) {
                    ...
                    if (!hashrate_str.empty()) {
                        ...
                        auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - g_stats.last_chart_update);
                        if (elapsed.count() >= 10) {
                            g_stats.hashrate_history.push_back(hashrate);
                            if (g_stats.hashrate_history.size() > 60) { // Keep last 60 points (10 minutes)
                                g_stats.hashrate_history.pop_front();
                            }
                            g_stats.last_chart_update = now;

                            // Refresh chart
                            if (g_chart_area) {
                                InvalidateRect(g_chart_area, nullptr, TRUE);
                            }
                        }
                    }
                }
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE: {
            ...
            // Create fonts
            g_font_title = CreateFont(20, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE,
            ...
            // Title
            CreateWindowA("STATIC", "XMRDesk Defender-Safe v5.3.0 - Enhanced Monero Mining",
                         WS_VISIBLE | WS_CHILD | SS_LEFT,
                         20, 20, 600, 25, hwnd, nullptr, nullptr, nullptr);
            ...
            g_wallet_edit = CreateWindowA("EDIT", "",
                                        ...
                                        20, 85, 680, 25, hwnd, (HMENU)ID_WALLET_EDIT, nullptr, nullptr);
            ...
            // Hashrate chart
            g_chart_area = CreateWindowA("STATIC", "",
                                       WS_VISIBLE | WS_CHILD | SS_OWNERDRAW,
                                       420, 190, 300, 135, hwnd, (HMENU)ID_CHART_AREA, nullptr, nullptr);
            ...
            g_log_listbox = CreateWindowA("LISTBOX", "",
                                        ...
                                        20, 365, 600, 120, hwnd, (HMENU)ID_LOG_LISTBOX, nullptr, nullptr);

            CreateWindowA("BUTTON", "Clear Log",
                         WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON,
                         630, 340, 90, 20, hwnd, (HMENU)ID_CLEAR_LOG_BUTTON, nullptr, nullptr);
            ...
            SendMessage(g_log_listbox, WM_SETFONT, (WPARAM)g_font_small, TRUE);
        ...
        case WM_DRAWITEM: {
            ...
            if (dis->CtlID == ID_CHART_AREA) {
                DrawHashrateChart(dis->hDC, dis->rcItem);
                return TRUE;
            }
            break;
        }
        ...
        case WM_PAINT: {
            ...
            // Draw Monero logo in upper right corner
            RECT logo_rect = {650, 60, 700, 110};
            DrawMoneroLogo(hdc, logo_rect);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pCmdLine, int nCmdShow) {
    ...
    const char CLASS_NAME[] = "XMRDeskImproved";
    ...
    // Create main window (wider)
    HWND hwnd = CreateWindowExA(
        0,
        CLASS_NAME,
        "XMRDesk Defender-Safe v5.3.0 - Enhanced Monero Mining Solution",
        WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX & ~WS_THICKFRAME,
        CW_USEDEFAULT, CW_USEDEFAULT, 760, 540,  // Wider window
