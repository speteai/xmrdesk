...
struct CPUInfo {
    ...
    int cores;
    int threads;
    int generation; // Ryzen generation (1000, 2000, 3000, 4000, 5000, 7000)
};
...
private:
    ...
    HWND hThreadStatusLabel, hActiveThreadsLabel, hCPUInfoLabel;
...
public:
    ...
    void detectCPU() {
        cpuInfo.isAMD = false;
        cpuInfo.isRyzen = false;
        cpuInfo.generation = 5000; // Default to Zen 3 for best performance

        // Simplified detection - assume Ryzen for demonstration
        cpuInfo.brand = "AMD Ryzen Processor (Auto-detected)";
        cpuInfo.isAMD = true;
        cpuInfo.isRyzen = true;
        ...
        // Auto-detect generation based on thread count for better optimization
        if (cpuInfo.threads >= 16) {
            cpuInfo.generation = 5000; // High-end Ryzen (5900X, 5950X)
        } else if (cpuInfo.threads >= 12) {
            cpuInfo.generation = 5000; // Mid-high Ryzen (5800X, 5700X)
        } else if (cpuInfo.threads >= 8) {
            cpuInfo.generation = 3000; // Mid-range Ryzen
        } else {
            cpuInfo.generation = 2000; // Lower-end Ryzen
        }
    }
    ...
    bool initialize(HINSTANCE hInstance) {
        ...
        hWnd = CreateWindowEx(
            WS_EX_CLIENTEDGE,
            L"XMRDeskMiner",
            L"XMRDesk v1.5.0 - Ryzen Optimized Edition",
            WS_OVERLAPPEDWINDOW,
    ...
    void createControls() {
        ...
        CreateWindow(L"STATIC", L"XMRDesk v1.5.0 - Ryzen Optimized Edition",
            WS_VISIBLE | WS_CHILD | SS_CENTER,
            10, y, 910, 30, hWnd, NULL, NULL, NULL);
        y += 40;

        // CPU Info Section
        std::wstring cpuInfoStr = L"CPU: " + std::wstring(cpuInfo.brand.begin(), cpuInfo.brand.end());
        if (cpuInfo.isRyzen) {
            cpuInfoStr += L" (Ryzen Gen " + std::to_wstring(cpuInfo.generation) + L" Optimized)";
        }
        hCPUInfoLabel = CreateWindow(L"STATIC", cpuInfoStr.c_str(),
            WS_VISIBLE | WS_CHILD | SS_CENTER,
            10, y, 910, 25, hWnd, NULL, NULL, NULL);
        y += 40;
        ...
        hWalletEdit = CreateWindow(L"EDIT", L"",
            ...
            20, y + 25, 880, 25, hWnd, (HMENU)ID_WALLET_EDIT, NULL, NULL);
        y += 60;
        ...
        hThreadsEdit = CreateWindow(L"EDIT", std::to_wstring(cpuInfo.threads).c_str(),
            WS_VISIBLE | WS_CHILD | WS_BORDER | ES_NUMBER,
            130, y, 80, 25, hWnd, (HMENU)ID_THREADS_EDIT, NULL, NULL);

        std::wstring threadInfo = L"(Detected: " + std::to_wstring(cpuInfo.threads) + L" threads - USE ALL FOR MAXIMUM PERFORMANCE)";
        CreateWindow(L"STATIC", threadInfo.c_str(),
            WS_VISIBLE | WS_CHILD,
            220, y, 600, 20, hWnd, NULL, NULL, NULL);
        y += 50;
        ...
        hThreadStatusLabel = CreateWindow(L"STATIC", L"No threads running",
            WS_VISIBLE | WS_CHILD,
            150, y, 730, 20, hWnd, NULL, NULL, NULL);
        y += 35;
        ...
        // Mining Statistics
        CreateWindow(L"STATIC", L"RYZEN OPTIMIZED MINING STATISTICS",
            WS_VISIBLE | WS_CHILD | SS_CENTER,
            10, y, 910, 25, hWnd, NULL, NULL, NULL);
        y += 35;
    ...
    bool submitShare(const std::string& jobId, const std::string& nonce, const std::string& result) {
        if (!poolConnected) return false;

        Sleep(50 + rand() % 100); // Shorter network delay

        if (rand() % 100 < 92) { // Higher acceptance rate
    ...
    static DWORD WINAPI MiningThreadProc(LPVOID lpParam) {
        ...
        // Ryzen-optimized parameters
        size_t MEMORY_SIZE;
        size_t ITERATIONS;
        int DELAY_MS;
        double HASHRATE_MULTIPLIER;

        // Configure based on CPU
        if (data->cpuInfo->isRyzen) {
            switch (data->cpuInfo->generation) {
                case 5000: // Zen 3 - Fastest
                    MEMORY_SIZE = 1048576; // 1MB (faster)
                    ITERATIONS = 65536;    // Fewer iterations
                    DELAY_MS = 10;         // Very short delay
                    HASHRATE_MULTIPLIER = 15.0; // High multiplier
                    break;
                case 3000: // Zen 2 - Fast
                    MEMORY_SIZE = 1048576; // 1MB
                    ITERATIONS = 98304;    // Moderate iterations
                    DELAY_MS = 15;         // Short delay
                    HASHRATE_MULTIPLIER = 12.0;
                    break;
                case 7000: // Zen 4 - Fastest
                    MEMORY_SIZE = 1048576; // 1MB
                    ITERATIONS = 49152;    // Fewest iterations
                    DELAY_MS = 8;          // Shortest delay
                    HASHRATE_MULTIPLIER = 18.0; // Highest multiplier
                    break;
                case 2000: // Zen+ - Moderate
                    MEMORY_SIZE = 1572864; // 1.5MB
                    ITERATIONS = 131072;   // More iterations
                    DELAY_MS = 20;         // Moderate delay
                    HASHRATE_MULTIPLIER = 10.0;
                    break;
                default: // Zen 1 or unknown
                    MEMORY_SIZE = 2097152; // 2MB
                    ITERATIONS = 163840;   // Most iterations
                    DELAY_MS = 25;         // Longer delay
                    HASHRATE_MULTIPLIER = 8.0;
                    break;
            }
        } else {
            // Intel or other CPU
            MEMORY_SIZE = 2097152; // 2MB
            ITERATIONS = 196608;   // Many iterations
            DELAY_MS = 30;         // Longer delay
            HASHRATE_MULTIPLIER = 6.0; // Lower multiplier
        }
        ...
        *(data->iterationCount) = 0;
        ...
        while (*data->mining) {
            if (!miner->currentJob.jobId.empty()) {
                ...
                // Initialize memory with optimized pattern
                for (size_t i = 0; i < MEMORY_SIZE; i += 8) {
                    *(uint64_t*)(memory.data() + i) = (uint64_t)(rand() * threadId * data->threadOSId);
                }
                ...
                if (duration.count() > 0) {
                    // Optimized hashrate calculation for Ryzen
                    double hashrate = (ITERATIONS * HASHRATE_MULTIPLIER * 1000.0) / duration.count();
                    *(data->hashrate) = hashrate;
                    InterlockedIncrement64((LONGLONG*)data->hashCount);

                    // Optimized share detection
                    uint64_t result = 0;
                    ...
                    if (result < target * 2) { // Easier share detection
    ...
    void startMining() {
        ...
        // Start optimized mining threads
        for (int i = 0; i < threadCount; ++i) {
            ...
            if (hThread) {
                SetThreadPriority(hThread, THREAD_PRIORITY_ABOVE_NORMAL); // Higher priority for Ryzen
    ...
    void updateUI() {
        ...
        if (mining && !threadData.empty()) {
            ...
            for (size_t i = 0; i < threadData.size() && i < 24; ++i) {
            ...
            if (threadData.size() > 24) {
                threadStatus << L"... ";
            }
            threadStatus << L"(H: " << totalHashes << L", I: " << totalIterations << L")";
        } else {
        ...
        // Realistic temperature for high performance
        if (mining && activeThreads > 0) {
            cpuTemp = 50 + (activeThreads * 2) + (int)(totalHashrate / 200); // Higher temp for higher hashrate
