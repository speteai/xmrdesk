...
public:
    bool start_mining(const std::string& wallet, const std::string& pool_host, int pool_port, int threads) {
        ...
        add_log("Mining started with XMRig", LogEntry::SUCCESS);
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE: {
            ...
            // Title
            CreateWindowA("STATIC", "XMRDesk Professional Miner v5.0.0 - NiceHash Style",
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pCmdLine, int nCmdShow) {
    ...
    const char CLASS_NAME[] = "XMRDeskProfessional";
    ...
    HWND hwnd = CreateWindowExA(
        0,
        CLASS_NAME,
        "XMRDesk Professional Miner v5.0.0 - Integrated XMRig",
