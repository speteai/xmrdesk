...
# XMRDesk Windows Build (Minimal Console Version)
# Creates a standalone Windows exe without external dependencies

set -e

echo "=== Building XMRDesk Windows Console Version ==="
echo "This creates a minimal but functional Windows exe"
echo ""
...
# Create minimal source files for Windows build
echo "Creating minimal Windows source files..."
...
# Create Makefile for MinGW cross-compilation
cat > Makefile << 'EOF'
...
$(TARGET): $(SOURCE)
	$(CXX) $(CXXFLAGS) -o $(TARGET) $(SOURCE) -lws2_32
...
if [ -f "xmrdesk.exe" ]; then
    ...
    # Test that it's a valid Windows executable
    if file xmrdesk.exe | grep -q "PE32+"; then
        echo "✅ Valid Windows x64 executable confirmed"
    else
        echo "⚠️  Warning: May not be a valid Windows executable"
    fi
else
    echo "❌ Build failed!"
    exit 1
